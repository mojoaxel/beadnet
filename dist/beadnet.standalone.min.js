(function (global, factory) {
	...
	function vopts(obj, defaults, options) {
	  ...
	  var objKeys = lodash.keys(obj);

	  var requiredKeys = [];

	  lodash.forOwn(defaults, function(value, key) {
	  ...
	  if (requiredKeys) {
	    var missing = lodash.difference(requiredKeys, objKeys);
	    if (missing.length > 0) {
	  ...
	  if (!options.allowUnknown) {
	    var extra = lodash.difference(objKeys, lodash.keys(defaults));
	    if (extra.length > 0) {
	      throw new Error('Unknown properties: ' + extra.join(', '));
	    }
	  }

	  return lodash.assign({}, defaults, obj);
	}


	var defaultOptions = function(obj, defaults, options) {
	...
	class BeatNet {
		...
		constructor(options) {
			...
			*/
			...
			loglevel.debug("initializing beadnet with options: ", this.opt);
		}
		...
		createSVG() {
			...
			loglevel.debug(`createSVG: width: ${parentRect.width}, height: ${parentRect.height}`);

			/* make sure size is valid */
			if (!width || !height) {
				loglevel.error(`createSVG: width: ${width}, height: ${height}`);
		...
		createNode({ id = randomID(), x = null, y = null0 }) {
			loglevel.debug(`createNode: id:${id}, x:${x}, y:${y}`);
			this._svg
				.selectAll(".point")
				.data(arguments)
				.enter()
		...
		createNodes(nodes) {
			loglevel.debug(`createNodes: ${nodes}`);
